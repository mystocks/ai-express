...
#define cam_err(format, ...)                                         \
  printf("[%s]%s[%d] E: " format "\n", __TIME__, __func__, __LINE__, \
         ##__VA_ARGS__)
#define cam_log(format, ...)                                         \
  printf("[%s]%s[%d] W: " format "\n", __TIME__, __func__, __LINE__, \
         ##__VA_ARGS__)
#if X2_CAM_DEBUG
...
extern int hb_cam_process(int event, void *arg);
...
extern int hb_cam_i2c_block_write(uint32_t port, int subdev, int reg_addr,
                                  char *buffer, int size);
extern int hb_cam_i2c_block_read(uint32_t port, int subdev, int reg_addr,
                                 char *buffer, int size);
extern int hb_cam_control_isp(uint32_t port, uint32_t enable);
extern int hb_cam_dynamic_switch(uint32_t port, uint32_t fps,
                                 uint32_t resolution);
extern int hb_cam_dynamic_switch_fps(uint32_t port, uint32_t fps);
extern int hb_cam_set_ex_gain(uint32_t port, uint32_t exposure_setting,
                              uint32_t gain_setting_0, uint16_t gain_setting_1);
extern int hb_cam_set_awb_data(uint32_t port, AWB_DATA_s awb_data,
                               float rg_gain, float b_gain);
