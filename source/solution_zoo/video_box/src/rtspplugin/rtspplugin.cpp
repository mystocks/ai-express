...
void RtspPlugin::Process() {
  ...
  for (int i = 0; i < channel_number_; ++i) {
    ...
    client = openURL(*env, "RTSPClient", rtsp_url_[i].c_str(),
                     ("channel" + std::to_string(i) + ".h264"));
...
void RtspPlugin::GetConfigFromFile(const std::string &path) {
  ...
  for (int i = 0; i < channel_number_; ++i) {
    ...
    LOGW << "rtsp info    " << rtsp_url;
    rtsp_url_.push_back(rtsp_url);
  }
}
