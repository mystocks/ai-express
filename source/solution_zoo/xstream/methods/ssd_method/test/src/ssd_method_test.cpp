...
TEST(SSD_TEST, ModelInfo) {
  const char *model_file = "config/models/ssd.hbm";
  const char *bpu_config = "config/bpu_config/bpu_config.json";

  BPUHandle bpu_handle;
  std::cout << "model_file" << model_file << std::endl;
  int ret = BPU_loadModel(model_file, &bpu_handle, bpu_config);
  EXPECT_EQ(ret, 0);
  ...
  model_info.Init(bpu_handle, model_name, &input_info, &output_info);
  std::cout << model_info;
}
