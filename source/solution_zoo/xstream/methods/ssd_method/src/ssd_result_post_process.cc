...
int GenShape(std::vector<std::pair<uint32_t, uint32_t>> &shapes,
             BPUModelInfo &output_info, int num_layers) {
  for (int i = 0; i < num_layers; i++) {
    int *valid_shape = output_info.valid_shape_array + i * 4;
    uint32_t h = valid_shape[1];
    uint32_t w = valid_shape[2];
...
int SsdAnchorsGenerator(std::vector<std::vector<Anchor>> &anchors_table,
                        ...
                        const SsdConfig &ssd_conf,
                        uint32_t num_outs, bool is_tf = false) {
...
int GetBboxFromRawData(BPUHandle bpu_handle, void *raw_box_data,
                       BPUModelInfo &output_info, int layer_num,
                       std::vector<Anchor> &anchors,
  ...
  int *aligned_shape = output_info.aligned_shape_array + layer_num * 4;
  int *valid_shape = output_info.valid_shape_array + layer_num * 4;
  int vecSize = output_layer_endian.size();
  bool big_endian = layer_num >= vecSize
                        ? false
                        : output_layer_endian[layer_num];  // layer_num从0开始
  uint8_t *shift_value = output_info.shift_value[layer_num];
  uint32_t nhwc[4];
  nhwc[0] = 1;
  nhwc[1] = aligned_shape[0] * aligned_shape[1];
  nhwc[2] = aligned_shape[2];
  nhwc[3] = aligned_shape[3];
  uint32_t real_hnum = valid_shape[1];
  uint32_t real_wnum = valid_shape[2];
  uint32_t real_cnum = valid_shape[3];
...
int SoftmaxFromRawScore(BPUHandle bpu_handle, void *raw_cls_data,
                        BPUModelInfo &output_info, int layer_num, int class_num,
                        double *cls_dest, float cut_off_threshold,
  ...
  int elem_type = output_info.dtype_array[layer_num];
  int elem_size = (elem_type == BPU_DTYPE_FLOAT32) ? 4 : 1;
  for (uint32_t hh = 0; hh < nhwc[1]; hh++) {
    uint32_t cur_h = hh % aligned_shape[1];
    if (cur_h >= real_hnum) continue;
    uint32_t res_id_cur_hh = hh * h_dest_stride;
    for (uint32_t ww = 0; ww < real_wnum; ww++) {
      ...
      for (uint32_t anchor_id = 0; anchor_id < anchor_num_per_pixel;
        ...
        //                for (int cls = 0; cls < class_num; ++cls) {
        //                  cls_dest[res_id_cur_anchor + cls] =
        //                  cls_dest[res_id_cur_anchor + cls] / sum; if
        //                  (cls_dest[res_id_cur_anchor + cls] <
        //                  cut_off_threshold) {
        //                    cls_dest[res_id_cur_anchor + cls] = 0;
        //                  }
        //                }
      }
    }
...
int SSDAux(BPUHandle bpu_handle, BPUModelInfo &output_info,
           BPU_Buffer_Handle const *output_base_vaddr, uint32_t layer_num,
  ...
  if (anchors_layers != layer_num) {
    ...
    for (uint32_t i = 0; i < layer_num; i++) {
      ...
      void *box_vaddr = BPU_getRawBufferPtr(output_base_vaddr[i + layer_num]);
      void *cls_vaddr = BPU_getRawBufferPtr(output_base_vaddr[i]);
      ...
      SoftmaxFromRawScore(bpu_handle, cls_vaddr, output_info, i,
                          ssd_conf.cls_num, cls_table, 0.01, score_threshold,
                          output_layer_endian);

      GetBboxFromRawData(bpu_handle, box_vaddr, output_info, i + layer_num,
                         anchors, dets, score_threshold, cls_table, ssd_conf,
