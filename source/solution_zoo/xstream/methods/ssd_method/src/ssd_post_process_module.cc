...
void SSDPostProcessModule::GetFrameOutput(
    const std::vector<BPU_Buffer_Handle> &input_buf, BaseDataPtr frame_output) {
  std::vector<Detection> result_vec;
  auto output = std::static_pointer_cast<BaseDataVector>(frame_output);

  SSDAux(bpu_handle_, output_info_, input_buf.data(), num_layer_,
...
int SSDPostProcessModule::Init() {
  ...
  if (ret != 0) {
    LOGF << "bpu get model output info failed" << std::endl;
  }

  num_layer_ = output_info_.num / 2;

  ret = xstream::LoadConfig(ssd_params_file_.c_str(), &ssd_conf_, num_layer_);
  if (ret != 0) {
  ...
  SsdAnchorsGenerator(anchors_table_, output_info_, ssd_conf_, output_info_.num,
                      is_tf_);
