...
void CommonLmkPostPredictor::HandleLmk(
    ...
    std::vector<BaseDataPtr> *output) {
  if (mxnet_outs.size()) {
    auto lmk = Lmk3Post(mxnet_outs, box, nhwc);
...
BaseDataPtr CommonLmkPostPredictor::Lmk3Post(
    ...
    const hobot::vision::BBox &box,
    const std::vector<std::vector<uint32_t>> &nhwc) {
  auto heatmap_pred = reinterpret_cast<const float *>(mxnet_outs[0].data());
  static const float diff_coeff = 1.0;
  ...
  for (auto k = 0; k < lmk_num_; ++k) {  // c
    ...
    y = y + (diff_y * diff_coeff + 0.5);
    x = x + (diff_x * diff_coeff + 0.5);
    y = y * i_o_stride_;
    x = x * i_o_stride_;
