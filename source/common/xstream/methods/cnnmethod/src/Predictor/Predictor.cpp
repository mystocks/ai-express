...
int32_t Predictor::Init(std::shared_ptr<CNNMethodConfig> config) {
  ...
  HOBOT_CHECK(ret == 0) << "get model output info failed, model:"
                        << model_name_;
  ret = BPU_getModelInputInfo(bpu_handle_, model_name_.c_str(),
                              &input_model_info);
  ...
  for (size_t i = 0; i < model_info_.output_layer_size_.size(); i++) {
...
int Predictor::RunModelFromImage(uint8_t *data,
  ...
  int ret =
      BPU_runModelFromImage(bpu_handle_,
                            model_name_.c_str(),
                            fake_img_ptr,
                            output_buf,
                            output_size,
...
int Predictor::RunModelFromResizer(
...
int Predictor::RunModelFromDDRWithConvertLayout(int8_t *data, int data_size,
                                                BPU_Buffer_Handle *output_buf,
                                                int output_size) {
  std::vector<BPU_Buffer_Handle> input_data;
  auto input_buf = BPU_createBPUBuffer(data, data_size);
  if (input_buf == nullptr) {
    LOGE << "Failed to alloc input buffer";
    return -1;
  }
  input_data.push_back(input_buf);

  BPUModelHandle model_handle;
  int ret =
      BPU_runModelFromDDRWithConvertLayout(bpu_handle_,
                                           model_name_.c_str(),
                                           input_data.data(),
                                           input_data.size(),
                                           output_buf,
                                           ...
                                           &model_handle);
  if (ret != 0) {
    LOGE << "BPU_runModelFromDDRWithConvertLayout failed: "
...
int Predictor::RunModelFromDDR(int8_t *input_buf, std::vector<int> input_size,
                               int ninput, BPU_Buffer_Handle output_buf[],
                               int noutput) {
  if (input_size.size() < 1) {
    return -1;
  }
  BPU_Buffer_Handle input_buffer =
      BPU_createBPUBuffer(input_buf, input_size[0]);
  std::vector<BPU_Buffer_Handle> input_data = {input_buffer};
  if (input_buffer == nullptr) {
    LOGE << "get ddr data failed";
    return -1;
  }
  BPUModelHandle model_handle;
  int ret =
      BPU_runModelFromDDR(bpu_handle_, model_name_.c_str(), input_data.data(),
                          ninput, output_buf, noutput, &model_handle);

  if (ret != 0) {
    BPU_freeBPUBuffer(input_buffer);
    LOGE << "BPU_runModelFromImage failed:" << BPU_getLastError(bpu_handle_);
    return -1;
  }
  ret = BPU_getModelOutput(bpu_handle_, model_handle);
