...
void VidInputPredictor::Do(CNNMethodRunData *run_data) {
  ...
  for (int batch_i = 0; batch_i < batch_size; batch_i++) {  // loop frame
    ...
    {
      int ret = RunModelFromDDR(
          cvt_output.data(), std::vector<int>({ddr_input_size}), 1,
          bufs.out_bufs_.data(), model_info_.output_layer_size_.size());
      if (ret == -1) {
        return;
      }
      ...
      for (int j = 0; j < layer_size; j++) {
        ...
        /*
        hbrtGetFeatureNativeNHWC(BPU_getRawBufferPtr(bufs.out_bufs_[j]),
                                 feature_bufs_[j].data(),
                                 model_info_.layout_flag_[j],
                                 model_info_.aligned_nhwc_[j].data());
        */
        BPU_convertLayout(bpu_handle_, feature_bufs_[j].data(),
                          ...
                          j, -1, -1, -1, -1);
        ConvertOutputToMXNet(feature_bufs_[j].data(), one_tgt_mxnet[j].data(),
                             j);
