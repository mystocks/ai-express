...
int QueueBuffer::enqueue(void *data, uint32_t size)
{
    ...
    print("Enqueue# in\n");
    pthread_mutex_lock(&mutex);
    LOGI << "uvc_queue num = "<<uvc_queue_->num << " total = "<<uvc_queue_->total;
    if (uvc_queue_->num >= uvc_queue_->total) {
        print("Enqueue# que is full. num(%u), total(%u)\n",
                uvc_queue_->num, uvc_queue_->total);
        pthread_mutex_unlock(&mutex);
        return -1;
    }
    ...
    print("Enqueue# _node@ %p, first@ %p, last@ %p, num@ %u, total@ %u\n",
            new_node, uvc_queue_->first, uvc_queue_->last, uvc_queue_->num, uvc_queue_->total);
    pthread_mutex_unlock(&mutex);
    print("Enqueue exit\n");
...
void* QueueBuffer::dequeue(uint32_t *size)
{
    ...
    LOGI << "dequeue num = "<<uvc_queue_->num << " total = "<<uvc_queue_->total;
    ...
    *size = _node->size;
    ...
    print("Free _node@ %p, first@ %p, last@ %p, num@ %u, total@ %u\n",
            _node, uvc_queue_->first, uvc_queue_->last, uvc_queue_->num, uvc_queue_->total);
    free(_node);
    ...
    dur = t2_ms - t1_ms;
    LOGD << "get data queue t1_ms(" << t1_ms << "ms) t2_ms("
         << t2_ms << "ms) interval (" << intv << "ms), and cost ("
         << dur << "ms)";
    t1_last = t1_ms;
    return data;
}
...
void QueueBuffer::queue_flush()
{
    ...
    print("Flush# first@ %p, last@ %p, num@ %u, total@ %u\n",
            uvc_queue_->first, uvc_queue_->last, uvc_queue_->num, uvc_queue_->total);
