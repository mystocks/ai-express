...
运行时，只需要在x2/x3设备上运行run.sh脚本，在PC上打开浏览器，看展示客户端即可。  
 ```
sh run.sh [ face | face_recog | body | xbox | behavior | video_box | guesture | face_body_multisource | ssd_test ] [ 96board | 2610 | x3dev | x3svb ] [ imx327 | os8a10 | s5kgm(only work for body and behavior) | s5kgm_2160p | hg | usb_cam ] [ cache | jpg | nv12 ]
 ```
...
| 手势识别     |  x2 96board |  sh run.sh guesture 96board | 不支持 | 浏览器输入XJ3的IP地址，点击预览即可 |
| 手势识别     |  x2 面板机 |  sh run.sh guesture 2610 | 不支持 | 浏览器输入XJ3的IP地址，点击预览即可|
| 手势识别     |  x3dev |  sh run.sh guesture x3dev  usb_cam/imx327/os8a10/s5kgm_2160p/s5kgm | sh run.sh guesture x3dev  hg  cache/jpg/nv12 | 浏览器输入XJ3的IP地址，点击预览即可 |
| 手势识别     |  x3svb |  sh run.sh guesture x3svb  usb_cam/os8a10 | sh run.sh guesture x3dev  hg  cache/jpg/nv12 | 浏览器输入XJ3的IP地址，点击预览即可 |
