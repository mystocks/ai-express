...
or you can just use docker image `docker.hobot.cc/rl/bpu_predict:0.0.1`. See [Docker readme](./scm/project/docker_image/README.md)
```bash
docker run -v $PROJECT_ROOT:/PROJECT_ROOT \
    -v ~/.ssh:/root/.ssh \
    -w /PROJECT_ROOT \
    /bin/bash
```
...
Please make sure your code follow the rules.
