...
usage(){
   ...
   echo "usage: sh run.sh [ face | face_recog | body | xbox | behavior | face_body_multisource | ssd_test ] [ 96board | 2610 | x3dev ] [ imx327 | os8a10 | s5kgm(only work for body and behavior) | s5kgm_2160p | hg ] [ cache | jpg | nv12 ]"
   exit 1
}
...
#        cp -rf vehicle_solution/configs/config_match_8M.json vehicle_solution/configs/config_match.json
        ...
        if [ "$source" == "cache" ]; then
            ...
            sed -i 's#\("data_source": \).*#\1"cached_image_list",#g' configs/vio_config.json.x3dev.iot.hg
        elif [ "$source" == "jpg" ]; then
            echo "data source is jpg image list"
            sed -i 's#\("data_source": \).*#\1"jpeg_image_list",#g' configs/vio_config.json.x3dev.iot.hg
            sed -i 's#\("file_path": \).*#\1"configs/vio_hg/name.list",#g' configs/vio_config.json.x3dev.iot.hg
        elif [ "$source" == "nv12" ]; then
            echo "data source is nv12 image list"
            sed -i 's#\("data_source": \).*#\1"nv12_image_list",#g' configs/vio_config.json.x3dev.iot.hg
            sed -i 's#\("file_path": \).*#\1"configs/vio_hg/name_nv12.list",#g' configs/vio_config.json.x3dev.iot.hg
        else
...
#    ./vehicle_solution/vehicle_solution $vio_cfg_file ./vehicle_solution/configs/vehicle_solution.json ./configs/visualplugin_vehicle.json -i normal
  ...
  then
    ...
    then
      sed -i 's#\("data_source": \).*#\1"cached_image_list",#g' configs/vio_config.json.x3dev.iot.hg
      face_body_multisource_vio=configs/vio_config.json.x3dev.iot.hg
    fi
